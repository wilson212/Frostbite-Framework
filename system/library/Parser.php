	...
	public function parse($source, $data)
	{
		...
		foreach($data as $key => $value)
		{
			...
			if(is_array($value))
			{
				...
				// Now that we are done checking for blocks, 
				// Create our array key indentifier
				$key = $key .".";
				...
				if(strpos($source, $this->l_delim . $key) !== FALSE)
				{
					...
					while(preg_match("~" . $regex . "~iUs", $source, $match))
					{
						...
						// Check for a false reading
						if($replacement === FALSE) $replacement = "<<!". $key . $match[1] ."!>>";
						$source = str_replace($match[0], $replacement, $source);
					}
				}
			}
	...
	public function parse_array($key, $array)
	{
		...
		if(strpos($key, '.') !== false)
		{
			...
			else
			{
				...
				show_error('unkown_template_var_in_array', array($last[0], "\$". $args[0]), E_WARNING);
			}
		}
		...
		{	
			...
			{
				show_error('unkown_template_var_in_array', array($key, "\$". key($array)), E_WARNING);
			}
		}
